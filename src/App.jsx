...
const problems = [
  "IT keeps breaking", "Worried about cybersecurity", "Microsoft 365 needs managing",
  "Network/Wi-Fi problems", "Need a business application", "Current IT provider isn't delivering",
];

// TODO(Mo): swap these for real, current figures before launch — placeholders will hurt trust if left in.
const metrics = [
  { label: "RESPONSE TIME", value: "< 1 HR", note: "priority SLA tickets" },
  { label: "ENDPOINTS MANAGED", value: "500+", note: "across active clients" },
  { label: "UPTIME", value: "99.9%", note: "managed infrastructure" },
  { label: "SUPPORT", value: "24/7", note: "monitoring & alerting" },
];

const whyUs = [
  { title: "Local", text: "Polokwane-based support with onsite capability." },
  { title: "Proactive", text: "We identify problems before they become downtime." },
  { title: "Security-first", text: "Security is built into the way we manage technology." },
  { title: "One Partner", text: "IT support, infrastructure, cloud and software under one roof." },
  { title: "Business-focused", text: "We care about your workflows and outcomes — not just hardware." },
];

// Prices are ZAR, excl. VAT — confirm current rates before publishing.
const plans = [
  {
    name: "ESSENTIALS", price: "From R2,500/mo", featured: false,
    desc: "For businesses that need reliable IT without employing a full-time IT team.",
    features: ["Remote support", "Microsoft 365 management", "Backup monitoring", "Endpoint management", "Basic security"],
    cta: "See if this plan fits →",
  },
  {
    name: "BUSINESS", price: "From R5,500/mo", featured: true,
    desc: "For growing businesses needing managed infrastructure and security.",
    features: ["Everything in Essentials", "Advanced cybersecurity", "Priority SLA"],
    cta: "See if this plan fits →",
  },
  {
    name: "ENTERPRISE", price: "Custom", featured: false,
    desc: "For complex environments and larger organisations.",
    features: ["Dedicated engineer", "Custom SLAs", "Full stack"],
    cta: "Talk to us →",
  },
];
...
/* ---------- Scroll-reveal hook (respects prefers-reduced-motion via CSS) ---------- */
...
function App() {
  ...
  useEffect(() => {
    ...
    const handleMotion = () => setPrefersReducedMotion(rmq.matches);
    handleMotion();
    rmq.addEventListener("change", handleMotion);
    ...
    return () => {
      ...
      rmq.removeEventListener("change", handleMotion);
    };
  }, []);
  ...
  const submit = useCallback(async (e) => {
    ...
    if (form.website) { setStatus("sent"); return; } // honeypot — silently "succeed" for bots
    ...
    <div className="app">
      ...
      <header className="nav">
        <a href="#home" className="brand">
          <img src="/logo.png" alt="Cyber I.T Masters" onError={(e) => (e.currentTarget.style.display = "none")} />
          <span><b>CYBER</b> I.T MASTERS<small>IT MSP · SOLUTIONS · SOFTWARE</small></span>
        </a>
        ...
        <nav ref={navRef} id="primary-navigation" className={menuOpen ? "nav-links open" : "nav-links"}>
          {["Services", "Solutions", "Pricing", "Case Studies", "Why Us", "Contact"].map((x) => (
            <a key={x} href={`#${x.toLowerCase().replace(" ", "-")}`} onClick={() => setMenuOpen(false)}>{x}</a>
          ))}
      ...
      <main id="main-content" tabIndex="-1">
          ...
          <div className="hero-grid" style={{ position: "relative", zIndex: 1 }}>
            <div className="hero-copy">
              ...
              <p className="hero-lead">
                IT support, cybersecurity and software for businesses that can't afford technology problems.
                <br /><br />
                Polokwane-based. Nationwide remote support. Enterprise-grade capability without enterprise complexity.
              </p>
              <div className="hero-actions">
                <a className="btn primary" href="#contact">Book a Technology Assessment</a>
                <a className="btn ghost" href="#services">Explore Services ↓</a>
              </div>
              <div className="trust-row">
                <span>✓ Local Support</span><span>✓ Security-first</span><span>✓ One technology partner</span>
              </div>
            </div>
        ...
        {/* Trust bar — restored from unused CSS; replace placeholder figures with real ones */}
        ...
        <Reveal as="section" id="solutions" className="section dark-section problems-section">
          ...
          <div className="problems-grid">
            {problems.map((p) => <div key={p} className="problem-card">⚠️ {p}</div>)}
          </div>
        ...
        <Reveal as="section" id="why-us" className="section">
          ...
          <div className="why-grid">
            {whyUs.map((w) => (
              <div key={w.title} className="why-card"><h3>{w.title}</h3><p>{w.text}</p></div>
            ))}
          </div>
        </Reveal>

        <Reveal as="section" id="pricing" className="section dark-section pricing-section">
          ...
          <div className="price-grid">
            {plans.map((p) => (
              <div key={p.name} className={p.featured ? "price-card featured" : "price-card"}>
                ...
                <div className="price-note">excl. VAT</div>
                <p>{p.desc}</p>
                <ul>{p.features.map((f) => <li key={f}>✓ {f}</li>)}</ul>
                <a className="btn ghost" href="#contact">{p.cta}</a>
              </div>
            ))}
          </div>
        </Reveal>

        <Reveal as="section" id="services" className="section">
        ...
        <Reveal as="section" id="solutions-block" className="section dark-section">
          ...
          <div className="cap-grid">
            {capabilities.map((c, i) => <div key={c}><span>{String(i + 1).padStart(2, "0")}</span><b>{c}</b></div>)}
          </div>
        </Reveal>

        <Reveal as="section" id="software" className="section">
          <div className="devops-grid">
            <div>
              <div className="eyebrow">06 / SOFTWARE & DEVOPS</div>
              <h2>FROM <em>IDEA</em> TO<br />PRODUCTION.</h2>
              <p className="large-copy">Need a client portal? Internal system? Business dashboard? Integration? We handle the architecture, development, deployment and ongoing support.</p>
              <a className="btn primary" href="#contact">Discuss a Software Project →</a>
            </div>
            <div className="terminal">
              <div className="terminal-bar"><span></span><span></span><span></span><b>deployment.pipeline</b></div>
...
STATUS:  ALL SYSTEMS GO`}</pre>
            </div>
          </div>
        </Reveal>

        <Reveal as="section" id="case-studies" className="section dark-section">
        ...
        <Reveal as="section" id="industries" className="section industry-section">
          ...
          <div className="industry-grid">
            {industries.map((x, i) => <div key={x}><span>0{i + 1}</span><b>{x}</b><small>Technology support & solutions</small></div>)}
          </div>
        </Reveal>

        <Reveal as="section" id="contact" className="section contact-section">
          <div className="contact-grid">
            <div>
              <div className="eyebrow">09 / START HERE</div>
              <h2>LET'S FIX<br /><em>YOUR IT.</em></h2>
              <p className="large-copy">Tell us what you are trying to improve, build or protect. We will route the enquiry to the right technology discipline.</p>
              <div className="contact-details">
                <a href="tel:+27726650565">072 665 0565</a>
                <a href="mailto:info@mbulahenigroup.co.za">info@mbulahenigroup.co.za</a>
                <span>Polokwane · Limpopo · South Africa</span>
              </div>
            </div>
            <form name="contact" onSubmit={submit} className="contact-form" noValidate>
              <label>Name<input name="name" required autoComplete="name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} /></label>
              <label>Company<input name="company" autoComplete="organization" value={form.company} onChange={(e) => setForm({ ...form, company: e.target.value })} /></label>
              <label>Email<input name="email" required type="email" autoComplete="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} /></label>
              <label>Phone / WhatsApp<input name="phone" required type="tel" autoComplete="tel" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} /></label>
              <label className="full">What can we help with?
              ...
              <button className="btn primary full" type="submit" disabled={status === "sending" || status === "sent"}>
              ...
              <div className={`form-status${status === "error" ? " error" : ""}`} role="status" aria-live="polite">
                ...
                {status === "error" && "Something went wrong. Please try again or call us."}
              </div>
            </form>
          </div>
        </Reveal>

        <Reveal as="section" id="faq" className="section faq-section">
          <div className="eyebrow">10 / FAQ</div><h2>QUESTIONS, <em>ANSWERED.</em></h2>
          <div className="faq-list">
            {faqs.map(([q, a], i) => (
              <div className={openFaq === i ? "faq open" : "faq"} key={q}>
                <button onClick={() => setOpenFaq(openFaq === i ? -1 : i)} aria-expanded={openFaq === i} aria-controls={`faq-${i}`}>
                  {q}<span>+</span>
                </button>
                <div id={`faq-${i}`} className="faq-answer"><div className="faq-answer-inner">{a}</div></div>
              </div>
            ))}
          </div>
        </Reveal>
      ...
      <footer>
        <div className="footer-brand">CYBER <em>I.T</em> MASTERS<small>MANAGED IT · SECURITY · CLOUD · SOFTWARE</small></div>
        <div className="footer-links"><a href="#services">Services</a><a href="#solutions">Solutions</a><a href="#software">Software</a><a href="#contact">Contact</a></div>
        <span>© {new Date().getFullYear()} Cyber I.T Masters · Mbulaheni Group</span>
      </footer>

      <a href="https://wa.me/27726650565" target="_blank" rel="noreferrer noopener" className="wa-float" aria-label="Chat on WhatsApp">
        <svg viewBox="0 0 32 32" fill="currentColor" width="30" height="30" aria-hidden="true">
